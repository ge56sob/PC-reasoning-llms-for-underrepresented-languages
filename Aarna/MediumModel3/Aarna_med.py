...
def detect_language_heuristic(text):
    ...
    en_words = ["the", "is", "are", "we", "solution", "answer", "therefore"]
...
def coherence_score(text):
    ...
    if any(k in text for k in ["therefore", "thus", "so", "hence", "step", "1.", "2."]):
        score += 0.4
...
def backtranslate_vi_to_en(question, tokenizer, model):
    prompt = f"""
Translate the following Vietnamese math problem into clear English.
Preserve all mathematical meaning exactly.
...
for lang in LANGUAGES:
    ...
    for split in SPLITS:
        ...
        total = len(dataset) if MAX_EXAMPLES_PER_SPLIT is None else min(MAX_EXAMPLES_PER_SPLIT, len(dataset))
        ...
        for i in range(total):
            ...
            print(f"Lang detected: {lang_detected} | Match: {lang_match}")
            print(f"Coherence: {coherence:.2f} | Reasoning length: {reason_len}")
            print("-" * 80)
